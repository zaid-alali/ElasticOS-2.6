...
ssize_t rawexpwrite(off_t a, char *buf, size_t len, CLIENT *client) {
    ...
    if (0)DEBUG4("(WRITE to fd %d offset %lu len %lu), ", fhandle, foffset, len);
...
ssize_t rawexpread(off_t a, char *buf, size_t len, CLIENT *client) {
    ...
    if (0) DEBUG4("(READ from fd %d offset %lu len %lu), ", fhandle, foffset, len);
...
#define sys_inject_page_nr 305
#define sys_get_page_nr 306

int save_it(int pid, unsigned long address, char *buf, unsigned int len) {
    return syscall(sys_inject_page_nr, pid, address, len, buf);
}

int bring_it(int pid, unsigned long address, char *buf, unsigned int len) {
    return syscall(sys_get_page_nr, pid, address, len, buf);
}
...
void negotiate(CLIENT *client) {
    ...
    cliserv_magic = cliserv_magic;
    printf("\n\n\nSending magic: %lu\n", cliserv_magic);
    if (write(client->net, &cliserv_magic, sizeof (cliserv_magic)) < 0)
        err("Negotiation failed: %m");
    size_host = (u64) (client->exportsize);
...
int mainloop(CLIENT *client) {
    ...
    printf("NBD_REPLY_MAGIC: (0x%lx), reply.magic: (0x%lx), (unsigned long)ntohl(reply.magic): (0x%lx)\n",
            (unsigned long) NBD_REPLY_MAGIC, (unsigned long) reply.magic,
    ...
    while (go_on) {
        ...
        //{
        if(0) printf("Received %s Request for pid(%d) and address(%lu)\n",
                (request.type == NBD_CMD_READ ? "READ" : (request.type == NBD_CMD_WRITE ? "WRITE" : "UNKNOWN")),
        ...
        if (request.type == NBD_CMD_WRITE) {
            ...
            if ((client->server->flags & F_READONLY) ||
                ...
                printf("[WRITE to READONLY!]\n");
                ERROR(client, reply, EPERM);
                continue;
            }
            if (save_it(request.pid, request.address, buf, len) < 0) {
                printf("Write failed: %m\n");
                ERROR(client, reply, -1);
                continue;
            }
            if ((0) && (expwrite(request.from, buf, len, client))) {
        ...
        //DEBUG("exp->buf, ");
        if(0) printf("Trying to read from address: %lu\n", request.address);
        if (bring_it(request.pid, request.address, buf + sizeof (struct nbd_reply), len) < 0) {
            printf("Read failed: %m\n");
            ERROR(client, reply, -1);
            continue;
        }
        if ((0) && expread(request.from, buf + sizeof (struct nbd_reply), len, client)) {
            DEBUG("Read failed: %m");
            ERROR(client, reply, errno);
            continue;
        }
...
void setupexport(CLIENT* client) {
    ...
    client->exportsize = getTotalSystemMemory();//laststartoff + lastsize;

    printf("\n\n\n\nExport size is: %lu\n\n\n", client->exportsize);
    printf("\n\n\n\nSys Mem size size is: %lu\n\n\n", getTotalSystemMemory());
...
int main(int argc, char *argv[]) {
    ...
    GArray *servers;
    GError *err = NULL;
    int nbd_req_size = sizeof ( struct nbd_request);

    if (nbd_req_size != 40) {
        fprintf(stderr, "Bad size of structure (Actual size: %d expected: %d). Alignment problems?\n", nbd_req_size, 28);
        exit(EXIT_FAILURE);
    }
