...
static __inline__ uint32_t get_intel_family_model(void)
{
...
int read_msr(int cpu, uint64_t *platform)
{
	uint64_t retval = 0;
	u32 data[2];
	u32 reg = 206;
	...
	if (err)
	{
		printk("rdmsr_safe_on_cpu error");
		return -1;
	}

	printk("rdmsr_safe_on_cpu returned %d and %d\n", data[0], data[1]);
...
uint32_t get_tsc_freq_khz(int cpu)
{
	...
	printk ("Detected processor with family/model of %x\n", get_intel_family_model());
	printk ("This is a %s processor with a base clock of %dkhz\n", tsc_processor_types[processor_type].name, tsc_processor_types[processor_type].base_clock_khz);

	if (processor_type == TOO_OLD) {
		printk("processor too new, too old, or not detected\n");
		return -1;
	}

	if (read_msr(cpu, &platform_info) == -1) {
		printk("error reading MSR_PLATFORM_INFO - Are you running this as root?\n");
		return -1;
	}

	printk("platform_info=%lld and non_turbo_ratio=%lld\n", platform_info, non_turbo_ratio);
...
static int __init pf_history_size(char *str)
{
	get_option(&str, &eos_ring_buffer_size);
	return 0;
}

early_param("pfhsize", pf_history_size);
...
asmlinkage void __init start_kernel(void)
{
	...
	uint64_t start, end;
	uint32_t tsc_in_hz = 0;
	uint32_t cycles_in_ns = 0;

	smp_setup_processor_id();

	//js = jiffies;
	//printk("Loc(%d): Jiffies: %lu\n", 1, jiffies);
	//RDTSC_START(start);
