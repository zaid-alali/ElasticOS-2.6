 ...
 * mistake.
 */
static void do_signal(struct pt_regs *regs_ptr)
{
	...
	//if(unlikely(test_thread_flag(TIF_MIGRATION)))
	//{
		//regs = task_pt_regs(current);
	//	clear_thread_flag(TIF_MIGRATION);
	//}
	...
	if (syscall_nr >= 0) {
		...
		err_nr = syscall_get_error(current, regs);
		switch (err_nr) {
		case -ERESTARTNOHAND:
		case -ERESTARTSYS:
		case -ERESTARTNOINTR:
