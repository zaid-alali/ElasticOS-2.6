...
int export_audit_context(struct checkpoint_dest_t *dest, struct task_struct *task)
{
	struct audit_context *ctx = task->audit_context;
	int err = 0;

	if (ctx)
		err = dest->write((char*) ctx, sizeof(*ctx));
